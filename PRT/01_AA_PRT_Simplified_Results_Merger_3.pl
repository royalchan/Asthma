...
####################################
# This script is to merge the 9 AA proteome files.
# Created by Rui Chen, 11/01/2015.
# Modified by Rui Chen, 12/06/2015.
...
my @files = ("PBMCAA1R1_Simplified.txt", "PBMCAA2R1_Simplified.txt", "PBMCAA3R1_Simplified.txt", "PBMCAA4R1_Simplified.txt", "PBMCAA5R1_Simplified.txt", "PBMCAA6R1_Simplified.txt", "PBMCAA7R1_Simplified.txt", "PBMCAA8R1_Simplified.txt", "PBMCAA9R1_Simplified.txt");
my $outfile = "AA_PRT_Combined_Results_20151206.txt";
...
for (my $i = 0; $i < scalar @files; ++$i) {
	...
	while ($line = <$fh>) {
		...
		# Define the expected number of elements (9 for each run).
		my $num = 9;
		if ($i == 0) {
			$cbd{$key} = "$reline";
		} elsif (defined $cbd{$key} || 0) {
			my @linearraycbd = split("\t", $cbd{$key});
			if (scalar @linearraycbd == $num * $i) {
				$cbd{$key} .= "\t$reline";			
			} else {
				do {push(@linearraycbd, ".");} until (scalar @linearraycbd == $num * $i);
...
foreach (@keys) {
	...
	if (scalar @linearray < 9 * (scalar @files)) {
		do {push(@linearray, ".");} until (scalar @linearray == 9 * (scalar @files));
	}
	my $reline = join("\t", @linearray);
