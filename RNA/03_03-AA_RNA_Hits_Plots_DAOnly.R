...
dat <- read.table("/Users/Rui/Rui Folders/Royal-Rock/Work/Snyder Lab Work Selected/Results/AA/AA_Methyl_Seq/Analyses/20141208-From_George/20150317-RNA_Seq_EpigenomicMarks/AA_RNA_Hits_Reordered_QN_DA.txt", sep = "\t", header = TRUE)
# Column Names.
...
# Heatmap -- NORMALIZED RAW DATA.
min(dat_data)
[1] -6.643856
max(dat_data)
[1] 10.77666
my_palette <- colorRampPalette(c("turquoise", "gray12", "gold"))(n = 500)
hc_ts <- hclust(dist(t(dat_data), method = "euclidean"), method = "ward.D")
total_heatmap <- heatmap.2(dat_data, Rowv=FALSE, Colv=as.dendrogram(hc_ts), dendrogram = "column", col = my_palette, margins=c(5,10), density.info = "none", trace = "none", scale = "none", symkey = FALSE, breaks = c(seq(-7,1,length=167), seq(1,3,length=167), seq(3,11,length=167)), ColSideColors = c(rep("red", 12), rep("green", 12)), keysize = 0.5, cexRow = 0.5)
total_heatmap <- heatmap.2(dat_data, Colv=FALSE, Rowv=FALSE, col = my_palette, margins=c(5,10), density.info = "none", trace = "none", scale = "none", symkey = FALSE, breaks = c(seq(-7,1,length=167), seq(1,3,length=167), seq(3,11,length=167)), ColSideColors = c(rep("red", 12), rep("green", 12)), keysize = 0.5, cexRow = 0.5)
total_heatmap <- heatmap.2(dat_data[,c(1,13,2,14,3,15,4,16,5,17,6,18,7,19,8,20,9,21,10,22,11,23,12,24)], Colv=FALSE, Rowv=FALSE, col = my_palette, margins=c(5,10), density.info = "none", trace = "none", scale = "none", symkey = FALSE, breaks = c(seq(-7,1,length=167), seq(1,3,length=167), seq(3,11,length=167)), ColSideColors = c(rep(c("red", "green"), 12)), keysize = 0.5, cexRow = 0.5)
total_heatmap <- heatmap.2(dat_data, Rowv=FALSE, Colv=as.dendrogram(hc_ts), dendrogram = "column", col = my_palette, margins=c(5,10), density.info = "none", trace = "none", scale = "row", symkey = FALSE, ColSideColors = c(rep("red", 12), rep("green", 12)), keysize = 0.5, cexRow = 0.5)
total_heatmap <- heatmap.2(dat_data, Colv=FALSE, Rowv=FALSE, col = my_palette, margins=c(5,10), density.info = "none", trace = "none", scale = "row", symkey = FALSE, ColSideColors = c(rep("red", 12), rep("green", 12)), keysize = 0.5, cexRow = 0.5)
total_heatmap <- heatmap.2(dat_data[,c(1,13,2,14,3,15,4,16,5,17,6,18,7,19,8,20,9,21,10,22,11,23,12,24)], Colv=FALSE, Rowv=FALSE, col = my_palette, margins=c(5,10), density.info = "none", trace = "none", scale = "row", symkey = FALSE, ColSideColors = c(rep(c("red", "green"), 12)), keysize = 0.5, cexRow = 0.5)


# Heatmap -- DIFFERENCES WITHIN TWIN PAIRS.
dat_data_da <- dat_data
dat_data_mean <- matrix(0,112,24)
for (j in 1:112) {
	for (i in 1:12) {
...
total_heatmap <- heatmap.2(dat_data_centered, Rowv=FALSE, Colv=as.dendrogram(hc_ts), dendrogram = "column", col = my_palette, margins=c(5,10), density.info = "none", trace = "none", scale = "none", symkey = FALSE, breaks = c(seq(-12,-0.1,length=167), seq(-0.1,0.1,length=167), seq(0.1,12,length=167)), ColSideColors = c(rep("red", 12), rep("green", 12)), keysize = 0.5, cexRow = 0.5)
total_heatmap <- heatmap.2(dat_data_centered, Colv=FALSE, Rowv=FALSE, col = my_palette, margins=c(5,10), density.info = "none", trace = "none", scale = "none", symkey = FALSE, breaks = c(seq(-12,-0.1,length=167), seq(-0.1,0.1,length=167), seq(0.1,12,length=167)), ColSideColors = c(rep("red", 12), rep("green", 12)), keysize = 0.5, cexRow = 0.5)
...
# Paired Test in Discordant Twins.
...
for (i in 1:length(dat_data[,1])) {
	t1 <- dat_data[i,1:12]
	t2 <- dat_data[i,13:24]
	test <- t.test(as.numeric(as.character(t1)), as.numeric(as.character(t2)), paired = TRUE, alternative = "two.sided")
	...
	for (j in 1:length(t1)) {
		refpsn <- c(t1[j], t2[j])
		lines(refpsn, type = "o", pch = ".", col = colr, lwd = 0.1, add = TRUE)
	}
}
...
for (i in 1:length(dat_data[,1])) {
	t1 <- dat_data[i,1:12]
	t2 <- dat_data[i,13:24]
	test <- wilcox.test(as.numeric(as.character(t1)), as.numeric(as.character(t2)), paired = TRUE, alternative = "two.sided")
	pv[i] <- round(test$p.value, digit = 3)
	tplot <- cbind(c(t1, t2), c(rep("Asthma", 12), rep("Healthy", 12)))
	boxplot(as.numeric(as.character(tplot[,1]))~as.character(tplot[,2]), main = paste(datrownames[i], ", p = ", pv[i]))
	if (mean(as.numeric(as.character(t1))) < mean(as.numeric(as.character(t2)))) {
...
# JUST FOR COMPARISON: UNPaired Test in Discordant Twins.
# Twelve Analytes -- T-Test.
par(mfrow = c(3,4))
pv <- c()
for (i in 1:length(dat_data[,1])) {
