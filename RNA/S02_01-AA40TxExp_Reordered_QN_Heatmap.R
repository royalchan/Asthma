...
dat <- read.table("/Users/Rui/Rui Folders/Royal-Rock/Work/Snyder Lab Work Selected/Results/AA/AA_RNA_Seq_20140630/20140630-NewestRound/Analyses/Combined/AA40TxExp_Reordered_QN.txt", header = TRUE, sep = "\t")
dat.m <- as.matrix(dat[,2:length(dat[1,])])
row.names(dat.m) <- dat[,1]

my_palette <- colorRampPalette(c("turquoise", "gray12", "gold"))(n = 500)
total_heatmap <- heatmap.2(dat.m, Colv=TRUE, Rowv=FALSE, dendrogram = "row", col = my_palette, margins=c(5,12), density.info = "none", trace = "none", scale = "none", keysize = 0.5)
hc_ts <- hclust(dist(t(dat.m), method = "euclidean"), method = "ward.D")
total_heatmap <- heatmap.2(dat.m, Colv=as.dendrogram(hc_ts), Rowv=FALSE, dendrogram = "column", col = my_palette, margins=c(5,12), density.info = "none", trace = "none", scale = "none", keysize = 0.5)

# colored dendrogram
source("http://addictedtor.free.fr/packages/A2R/lastVersion/R/code.R")
op = par(bg = "#EFEFEF")
A2Rplot(hc_ts, k = 38, boxes = FALSE, col.up = "gray50", col.down = c("#FF6B6B", "#4ECDC4", "#556270", "#0000FF", "#008000", "#FF00FF", "#008080", "#000000", "#800000", "#000080", "#808000", "#C0C0C0", "#00FF00", "#FF0000", "#ff9900", "#ff6666", "#6633ff", "#CDCD00", "#EE5C42", "#EEC900", "#FF3030", "#104E8B", "#B23AEE", "#458B74", "#8B7355", "#66CD00", "#CD1076", "#7CCD7C", "#CD1076","#EE7AE9", "#CD9B9B", "#A1A1A1", "#CDC673", "#68838B","#8968CD", "#778899", "#00C5CD", "#CD3278"))
# another colored dendrogram
op = par(bg = "gray15")
cols = hsv(c(0, 0.03, 0.05, 0.08, 0.1, 0.12, 0.15, 0.18, 0.2, 0.22, 0.25, 0.28, 0.3, 0.32, 0.35, 0.38, 0.4, 0.42, 0.45, 0.48, 0.5, 0.52, 0.57, 0.6, 0.62, 0.65, 0.68, 0.7, 0.72, 0.75, 0.78, 0.8, 0.82, 0.85, 0.88, 0.9, 0.92, 0.95), 1, 1, 0.8)
A2Rplot(hc_ts, k = 38, boxes = FALSE, col.up = "gray50", col.down = cols)





# Code.R from source: "http://addictedtor.free.fr/packages/A2R/lastVersion/R/code.R"
